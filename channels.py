...
class SubscriptionsCase(BaseCase):
    ...
    #     channel_page = ChannelPage(self.browser)
    #     channel_page.open(id = self.CHANNEL_ID)
    #     channel_page.subscribe()
    #     self.assertion.true(channel_page.is_subscribe() )
    #     channel_page.unsubscribe()
    #     self.assertion.true(channel_page.is_not_subscribe() )#TODO

    def test_subscribe_from_video_page(self):
        video_page = VideoPage(self.browser)
        video_page.open(id=self.VIDEO_ID)
        video_page.subscribe()
        self.assertion.true(video_page.is_subscribe())
        video_page.unsubscribe()
        self.assertion.true(video_page.is_not_subscribe())  # TODO
